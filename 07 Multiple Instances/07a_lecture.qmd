...
knitr::opts_knit$set(
  root.dir = here::here("07 Multiple Instances/")
)
...
- Some packages provide an infrastructure to deal with multiple animals (e.g. `adehabitatHR`, `move2`). 
...
::: {.fragment}
A `for`-loop has one variable, that changes for each iteration. 
...
The body of the looped (everything between `{ ` and `}`) is executed **four** times (there are four individuals). Each time the value of `i` changes.
...
::: {.columns}
::: {.column width = "50%"}
`lapply()` applies a function to each element of a list (or a vector) and returns list.
...
::: {.fragment}
Here it would make more sense to use `sapply` (R tries to simplify the result; here a vector instead a list).
...
The same, but saying which columns to keep: 
...
Lets come back to our example: *We want to calculate the mean step length of each fisher from the `amt_fisher` data set*
