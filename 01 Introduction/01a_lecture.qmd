...
- We split these 4h block into 3 - 4 chunks, each roughly structured like this: 
  - Lecture ~ 30 min
  - R walkthrough ~ 20 min
  - Introduction of exercises ~ 5 min
- A 20 min break between approx. from 15:50 to 16:10.
...
:::: {.notes}
This means, eventhough our computers differs, relative to the project root, we all have the same directory structure. 
::::
...
::: {.notes}
1. Preprocessing: E.g., Global Location Sensos only provide light intensity. Brian will talk about this a bit next. 
...
- Than we use tracking data and so on
...
- Projected CRS flatten the three dimensional data to the a two-dimensional plane (and introduce some distortion).
...
[^1]:[EPSG stands for European Petrol Survey Group, who came up with the system]{.fragment}
...
## Reading prepcossed movement data in R
...
- Creat an object, that knows your data are points in time and space. 
