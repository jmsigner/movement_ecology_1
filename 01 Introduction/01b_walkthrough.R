...
# 4. - 6. The `sex`, `id` and `name` of the animal each relocation belongs to.
...
# Note, if you use `read_csv()` from the `readr` package (which is loaded with
# the `tidyverse` package), the date is automatically parsed (i.e., R already
# "knows" this is a date). This is one advantage of the functions from the
# `readr` package. Note also that `read_csv()` returns a tibble while
