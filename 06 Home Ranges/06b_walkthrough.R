...
# We can manipulate these using the `terra` package, including exporting them
...
# There is a method for 'as.data.frame()' for a `SpatRaster` object, which
...
ggplot(ud_df, aes(x = x, y = y, fill = lyr.1)) +
  geom_raster()

# Let's make a plot with our 50% and 95% isopleths drawn over the UD raster.

ggplot() +
  geom_raster(data = ud_df, aes(x = x, y = y, fill = lyr.1)) +
