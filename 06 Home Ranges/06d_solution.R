...
#---------------Module 06 -- Home Ranges---------------X
#----------------Last updated 2022-01-06---------------X
#-------------------Exercise Solution------------------X
#######################################################X
...
(ar <- nd %>%
  ...
  dplyr::select(c(id:sex, mcp95:kde50)) %>%
  mutate(across(mcp95:kde50, \(x) units::set_units(x, "km^2"))))

# Alternatively, using map():
# Return a list rather than a single data.frame
a_mcp <- map(nd$mcp, \(x) hr_area(x, units = TRUE))
...
poly <- nd %>%
  ...
  dplyr::select(id:track, mcp_poly:kde_poly)
